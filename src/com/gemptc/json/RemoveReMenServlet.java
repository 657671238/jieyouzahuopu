 ...
 */
@WebServlet("/removeReMen")
public class RemoveReMenServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public RemoveReMenServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(pro_id!=null&&!pro_id.trim().equals("")) {
			...
			try {
				...
				if(p!=null) {
					//开始移除
					if(dao.removeReMen(pro_id)) {
						JSONResult.JSONReturnWithData("0", "移除成功", response);
					}else {
